...
let _id=0; const uid=()=>`i${++_id}`;

// ─── Brand ────────────────────────────────────────────────────────────────────
const N = “#002677”; // Equitable navy
const B = “#0047BB”; // mid blue
const S = “#E8EDF8”; // surface
const BD = “#D1D9ED”; // border
const MU = “#6B7A99”; // muted
const TX = “#0D1B3E”; // text
const GR = “#059669”; // green
const AM = “#D97706”; // amber
const RD = “#DC2626”; // red
const PT = “#0047BB”; // pretax
const RT = “#059669”; // roth
const TXC= “#D97706”; // taxable
const PS = “#7C3AED”; // pension
const SS = “#0F766E”; // ss
...
return{ id:uid(),pid,type,label:ATYPES[type].label,
taxType:type===“brok”?“taxable”:type===“rira”?“roth”:“pretax”,
balance:0, empPreTaxPct:4,empPreTaxDollar:400,empPreTaxMode:“pct”,
...
function mkPerson(n){ const p={id:uid(),name:n===1?“You”:`Person ${n}`,age:35,retireAge:65,lifeExp:90,salary:n===1?85000:72000,accounts:[]}; const a=mkAcct(“401k”,p.id); p.accounts=[a]; return p; }
...
const tot=ep+er; const cap=ms*((a.erCap||0)/100);
...
const r=rate/100/12; let b=Math.max(0,bal||0); const out=[Math.round(b)];
for(let y=1;y<=yrs;y++){for(let m=0;m<12;m++) b=b*(1+r)+dep; out.push(Math.round(Math.max(0,b)));}
return out;
}
function drawdown(bal,wPct,gPct,yrs){
const r=gPct/100/12; let b=Math.max(0,bal||0); const out=[Math.round(b)];
for(let y=1;y<=yrs;y++){const mw=b*(wPct/100)/12;for(let m=0;m<12;m++) b=Math.max(0,b*(1+r)-mw); out.push(Math.round(b));}
return out;
}
// Required monthly savings to hit a target balance in N years at r% annual
...
<input type=“number” value={value} min={min} max={max} step={step} onChange={e=>onChange(Number(e.target.value))} style={wide?{minWidth:80}:{}}/>
{suffix&&<span className="ni-b">{suffix}</span>}
</div>
);
}
function Sl({value,onChange,min,max,step,color=B}){
const p=cl((value-min)/(max-min)*100,0,100).toFixed(1);
return <input type=“range” min={min} max={max} step={step} value={value} onChange={e=>onChange(cl(+e.target.value,min,max))} style={{”–p”:`${p}%`,”–c”:color}}/>;
}
...
<input type=“range” min={min} max={max} step={step} value={value} onChange={e=>onChange(cl(+e.target.value,min,max))} style={{”–p”:`${p}%`,”–c”:color}}/>
</div>
);
}
...
<div className="seg">
{opts.map(o=>(
<button key={o.v} className={“sb”+(val===o.v?” on”:””)}
...
<span style={{position:“relative”,display:“inline-block”,width:38,height:21,flexShrink:0}}>
<input type=“checkbox” checked={on} onChange={e=>onChange(e.target.checked)} style={{opacity:0,width:0,height:0,position:“absolute”}}/>
...
{label&&<span style={{fontSize:”.82rem”,color:TX,fontWeight:500}}>{label}</span>}
</label>
);
}
...
{items.length>1&&<div style={{borderTop:`1px solid ${BD}`,marginTop:5,paddingTop:5,fontFamily:”‘JetBrains Mono’,monospace”,fontWeight:700,color:N}}>Total: {fmtK(total)}</div>}
</div>
);
}
...
return(
<div id={id} style={{background:”#fff”,border:`1px solid ${BD}`,borderRadius:12,marginBottom:“1.25rem”,overflow:“hidden”,boxShadow:“0 1px 4px rgba(0,38,119,.05)”}}>
{title&&(
...
<div>
<div style={{fontWeight:700,fontSize:”.9rem”,color:N,letterSpacing:”-.01em”,display:“flex”,alignItems:“center”,gap:7}}>
...
// ─── Wealth Map ───────────────────────────────────────────────────────────────
const WMC={retirement:”#1D4ED8”,nonretire:”#059669”,income:”#7C3AED”,liability:”#DC2626”,insurance:”#0F766E”,realestate:”#92400E”};
function WMCard({label,sub,value,type,negative}){
const bdr=WMC[type]||WMC.nonretire;
return(
<div style={{border:`1.5px solid ${bdr}`,borderRadius:6,padding:“5px 9px”,background:”#fff”,minWidth:100,maxWidth:138,boxShadow:“0 1px 4px rgba(0,0,0,.06)”,textAlign:“center”}}>
...
<div style={{fontFamily:”‘JetBrains Mono’,monospace”,fontWeight:700,fontSize:”.8rem”,color:negative?WMC.liability:bdr}}>
{negative&&”(”}{fmtK(Math.abs(value||0))}{negative&&”)”}
</div>
</div>
);
}
...
function WealthMap({persons,allA,streams,baseTotal,basePT,baseRT,baseTX,pension,pensionMo,ss,ssMo,totalNetAnnual}){
...
{[[“Retirement Asset”,WMC.retirement],[“Non-Retirement”,WMC.nonretire],[“Income”,WMC.income],[“Gov’t/Insurance”,WMC.insurance],[“Liability”,WMC.liability]].map(([l,c])=>(
...
{p.accounts.map(a=>(
<WMCard key={a.id} label={a.label} sub={a.taxType===“roth”?“Roth”:a.taxType===“taxable”?“Taxable”:“Pre-Tax”} value={a.balance||0} type={[“401k”,“tira”].includes(a.type)?“retirement”:a.type===“rira”?“retirement”:“nonretire”}/>
))}
</React.Fragment>
))}
...
{persons.map((p,i)=><PersonIcon key={p.id} name={p.name} color={[N,WMC.income,”#DB2777”][i%3]} size={44}/>)}
</div>
...
{!pension.on&&!ss.on&&<div style={{fontSize:”.7rem”,color:MU,fontStyle:“italic”,padding:8}}>Enable pension or SS<br/>to display here</div>}
</div>
)}
</div>
</div>
...
const ratio=need>0?cl(have/need,0,1.2):1;
const pctFilled=cl(ratio*100,0,100);
const over=have>need;
const color=over?GR:pctFilled>66?AM:RD;
return(
...
<span style={{fontSize:”.73rem”,fontFamily:”‘JetBrains Mono’,monospace”,fontWeight:700,color}}>{over?“✓ “:””}{pct(pctFilled,0)} funded</span>
</div>
<div style={{height:8,background:BD,borderRadius:4,overflow:“visible”,position:“relative”}}>
<div style={{height:“100%”,width:`${pctFilled}%`,background:color,borderRadius:4,transition:“width .5s ease”,position:“relative”}}>
{pctFilled>0&&pctFilled<=98&&<span style={{position:“absolute”,right:-1,top:-4,width:16,height:16,borderRadius:“50%”,background:color,border:“2px solid #fff”,boxShadow:“0 1px 4px rgba(0,0,0,.15)”}}/>}
</div>
</div>
...
// Goal income (backward mode)
const [goalMode,setGoalMode]  = useState(“estimate”); // “estimate” | “manual”
const [goalManual,setGoalManual] = useState(80000);
const [goalSpendPct,setGoalSpendPct] = useState(80);
...
let ptY=new Array(N+1).fill(0),rtY=new Array(N+1).fill(0),txY=new Array(N+1).fill(0);
allA.forEach(a=>{
const s=acctStreams(a,a._p.salary); const g=a.growth||7;
const tot=Math.max(s.total,0.001);
const ptR=s.pt/tot, rtR=s.rt/tot, txR=s.tx/tot;
const b=a.balance||0;
const ptA=projectYears(b*ptR,s.pt,g,N);
const rtA=projectYears(b*rtR,s.rt,g,N);
const txA=projectYears(b*txR,s.tx,g,N);
...
const basePT   = last.PT||0, baseRT=last.RT||0, baseTX=last.TX||0;
const baseTotal= last.TOT||0;

// ── Lifetime rows ─────────────────────────────────────────────────────────────
const lifeRows = useMemo(()=>{
if(!accumRows.length) return [];
const retY=maxRetYrs, totY=Math.max(maxLifeYrs,retY+2);
const rows=[];
for(let y=0;y<=retY;y++){
const r=accumRows[y]||{};
rows.push({age:p0.age+y,phase:“accum”,PT:r.PT||0,RT:r.RT||0,TX:r.TX||0,TOT:r.TOT||0});
}
const dY=totY-retY;
...
for(let k=1;k<=dY;k++){
rows.push({age:p0.age+retY+k,phase:“drawdown”,
PT:ptD[k]||0,RT:rtD[k]||0,TX:txD[k]||0,TOT:(ptD[k]||0)+(rtD[k]||0)+(txD[k]||0)});
}
return rows;
...
const tot=baseTotal; if(!tot) return{pt:0,rt:0,tx:0,net:0};
const gPT=tot*(wRate/100)*(basePT/tot), gRT=tot*(wRate/100)*(baseRT/tot), gTX=tot*(wRate/100)*(baseTX/tot);
const nPT=gPT*(1-taxRate/100), nRT=gRT, nTX=gTX*(1-ltcgRate/100);
...
// Estimate: base on spending categories
const monthly = goalHousing + goalHealth + goalTravel + goalOther;
// Gross up for taxes (pretax income needed)
const annualNet = monthly*12;
// Inflate to retirement
const inflated = annualNet * Math.pow(1+goalInflation/100, maxRetYrs);
return inflated;
},[goalMode,goalManual,goalHousing,goalHealth,goalTravel,goalOther,goalInflation,maxRetYrs]);

const goalPctOfSalary = useMemo(()=>{
if(goalMode===“manual”) return totalSal>0?goalManual/totalSal*100:80;
return goalSpendPct;
},[goalMode,goalManual,goalSpendPct,totalSal]);

// Required portfolio to fund goal (net) from investments only (ex pension/SS)
const pensionSsAnnual = (pensionMo+ssMo)*12;
const portionFromPortfolio = Math.max(0,goalEstimated - pensionSsAnnual);
// Gross up for blended tax rate
const blendedTax = (basePT+baseTX>0) ? ((basePT*taxRate/100 + baseTX*ltcgRate/100)/(basePT+baseTX+Math.max(baseRT,1)))*100 : taxRate;
const grossPortionNeeded = portionFromPortfolio / (1 - blendedTax/100*0.5); // rough gross-up
const requiredPortfolio  = grossPortionNeeded / (wRate/100);

const gap = requiredPortfolio - baseTotal;
const gapIncome = goalEstimated - totalNetAnnual;
const onTrack = gap <= 0;

// Required additional monthly savings to close gap
const currentBal = allA.reduce((s,a)=>s+(a.balance||0),0);
const reqMonthly = requiredMonthlySaving(requiredPortfolio, currentBal, 7, maxRetYrs);
const currentMonthly = streams.total;
const additionalNeeded = Math.max(0, reqMonthly - currentMonthly);

// ── Comprehensive What-If scenarios ──────────────────────────────────────────
// Helper: given a scenario’s params, compute portfolio & income at retirement
const scenarioResult = useCallback((extraMonthly=0, lumpSum=0, extraYears=0, growthAdj=0, retireAgeOverride=null)=>{
const yrs = retireAgeOverride ? Math.max(retireAgeOverride-p0.age,1) : maxRetYrs+extraYears;
const g = 7+growthAdj;
const ptProj = projectYears(basePT+lumpSum*(basePT/Math.max(baseTotal,1)), streams.pt+extraMonthly*0.6, g, yrs);
const rtProj = projectYears(baseRT+lumpSum*(baseRT/Math.max(baseTotal,1)), streams.rt+extraMonthly*0.3, g, yrs);
const txProj = projectYears(baseTX+lumpSum*(baseTX/Math.max(baseTotal,1)), streams.tx+extraMonthly*0.1, g, yrs);
const tot=(ptProj[yrs]||0)+(rtProj[yrs]||0)+(txProj[yrs]||0);
const inc=tot*(wRate/100)*(1-blendedTax/100*0.5)+(pensionMo+ssMo)*12;
return{portfolio:tot,income:inc,yearsToRetire:yrs};
},[basePT,baseRT,baseTX,baseTotal,streams,maxRetYrs,wRate,blendedTax,pensionMo,ssMo,p0.age]);

// Lump sum what-if state
const [wiLumpSum, setWiLumpSum] = useState(25000);
const [wiExtraMonthly, setWiExtraMonthly] = useState(0);
const [wiTab, setWiTab] = useState(“monthly”); // monthly | lumpsum | retire | growth | combined

const baseResult = useMemo(()=>scenarioResult(0,0,0,0),[scenarioResult]);

const whatIfMonthly = useMemo(()=>{
const steps=[0,100,200,300,500,750,1000,1500,2000];
return steps.map(extra=>{
const r=scenarioResult(extra,0,0,0);
return{extra,portfolio:r.portfolio,income:r.income};
});
},[scenarioResult]);

const whatIfLumpSum = useMemo(()=>{
const steps=[0,5000,10000,25000,50000,100000,250000];
return steps.map(lump=>{
const r=scenarioResult(0,lump,0,0);
...
const ages=[base-5,base-2,base,base+2,base+5,base+8,base+10].filter(a=>a>p0.age);
return ages.map(age=>{
const r=scenarioResult(0,0,0,0,age);
return{age,yrs:age-p0.age,portfolio:r.portfolio,income:r.income};
});
},[scenarioResult,p0.retireAge,p0.age]);

const whatIfGrowth = useMemo(()=>{
const adjs=[-2,-1,0,1,2,3];
return adjs.map(adj=>{
const r=scenarioResult(0,0,0,adj,null);
return{adj,rate:7+adj,portfolio:r.portfolio,income:r.income};
});
},[scenarioResult]);

const whatIfCombined = useMemo(()=>{
// Show extra monthly + one-time cash deployed together at various levels
const combos=[
...
return combos.map(c=>{
const r=scenarioResult(c.extra,c.lump,0,0);
...
input[type=range]::-webkit-slider-thumb{-webkit-appearance:none;width:16px;height:16px;border-radius:50%;background:var(--c,${B});border:2.5px solid #fff;box-shadow:0 0 0 2px var(--c,${B})44,0 1px 4px rgba(0,0,0,.1);transition:box-shadow .15s}
...
.hint.g{color:${GR};font-weight:600}
.hint.r{color:${RD};font-weight:600}
...
.btn{display:inline-flex;align-items:center;gap:.35rem;padding:.4rem .9rem;border-radius:7px;border:1.5px solid;font-size:.78rem;font-weight:600;cursor:pointer;font-family:'DM Sans',sans-serif;transition:all .15s;line-height:1}
.btn-primary{background:${N};border-color:${N};color:#fff}.btn-primary:hover{background:#001a5c}
.btn-outline{background:#fff;border-color:${BD};color:${TX}}.btn-outline:hover{border-color:${B};color:${B}}
.btn-ghost{background:transparent;border-color:${BD};color:${MU}}.btn-ghost:hover{border-color:${B};color:${B}}
.btn-rm{background:transparent;border:1.5px solid #FECACA;color:${RD};border-radius:6px;padding:.2rem .6rem;font-size:.7rem;cursor:pointer;transition:all .15s;font-family:'DM Sans',sans-serif}.btn-rm:hover{background:#FEF2F2}
...
.ac-dot{width:8px;height:8px;borderRadius:2px;background:var(--c);flex-shrink:0}
.ac-name{font-weight:700;font-size:.85rem;color:${N}}
.ac-rm{background:none;border:none;color:${MU};cursor:pointer;font-size:.78rem;opacity:.4;transition:opacity .15s;padding:1px 4px;margin-left:auto}.ac-rm:hover{opacity:1;color:${RD}}
...
/* big number */
.big-num{font-family:'JetBrains Mono',monospace;font-weight:700;color:${N}}
...
/* gap analysis */
.gap-banner{border-radius:12px;padding:1.25rem 1.5rem;margin-bottom:1rem}
.gap-banner.success{background:linear-gradient(135deg,#F0FDF4,#ECFDF5);border:1.5px solid #6EE7B7}
.gap-banner.warn{background:linear-gradient(135deg,#FFF7ED,#FEF2F2);border:1.5px solid #FCD34D}
.gap-banner.danger{background:linear-gradient(135deg,#FEF2F2,#FFF1F2);border:1.5px solid #FECACA}
...
/* what-if table */
.wif-row{display:grid;grid-template-columns:1.2fr 1.5fr 1.5fr auto;gap:.5rem;padding:.55rem .8rem;border-radius:7px;align-items:center;font-size:.78rem}
.wif-row:nth-child(even){background:${S}}
.wif-row.head{background:${N};color:#fff;font-size:.68rem;font-weight:700;text-transform:uppercase;letter-spacing:.04em;border-radius:8px}
.wif-row.active{background:#EFF6FF;border:1.5px solid #BFDBFE}

/* asset map */
.am-wrap{display:flex;align-items:center;gap:1rem;flex-wrap:wrap;position:relative;justify-content:center;padding:1rem 0}
.am-line{position:absolute;top:50%;left:0;right:0;height:2px;background:${BD};z-index:0}
...
.section-chip{display:inline-flex;align-items:center;gap:5px;background:${N};color:#fff;font-size:.67rem;font-weight:700;text-transform:uppercase;letter-spacing:.06em;padding:3px 10px;border-radius:99px;margin-bottom:.7rem}
...
@media(max-width:680px){.g2,.g3,.gAuto{grid-template-columns:1fr}}
```
  ...
  <div className="topbar">
    <div className="topbar-inner">
      <div className="logo-box">
        ...
        <div><div className="logo-text">Retirement Planner</div><div className="logo-sub">All-in-one planning tool</div></div>
      </div>
...
<div style={{flex:1}}>
<div style={{fontWeight:700,fontSize:“1rem”,color:N}}>Forward Planning — What Can I Afford?</div>
<div style={{fontSize:”.78rem”,color:MU,marginTop:2}}>Enter everything you have and are doing. We’ll show you exactly what your retirement will look like and what changes would mean.</div>
</div>
...
<div className=“sv” style={{color:GR,fontSize:“1.1rem”}}>{fmt(totalNetMonthly)}</div>
</div>
</div>
</div>
):(
<div style={{background:”#fff”,border:`1px solid ${BD}`,borderRadius:10,padding:”.9rem 1.25rem”,marginTop:“1.25rem”,marginBottom:”-.25rem”,display:“flex”,alignItems:“center”,gap:“1rem”,flexWrap:“wrap”}}>
<div style={{flex:1}}>
<div style={{fontWeight:700,fontSize:“1rem”,color:N}}>Backward Planning — What Do I Need?</div>
<div style={{fontSize:”.78rem”,color:MU,marginTop:2}}>Start with your income goal. We’ll show you what it requires, what you have, and exactly what to do differently.</div>
</div>
...
<div className="sl">Goal Income</div>
<div className=“sv” style={{color:N,fontSize:“1.1rem”}}>{fmt(goalEstimated/12)}/mo</div>
</div>
<div className=“stat” style={{padding:”.6rem 1rem”,textAlign:“center”,borderColor:onTrack?GR+“55”:AM+“55”,background:onTrack?”#F0FDF4”:”#FFFBEB”}}>
<div className="sl">Gap</div>
<div className=“sv” style={{color:onTrack?GR:RD,fontSize:“1.1rem”}}>{onTrack?“On Track”:“−”+fmtK(Math.abs(gap))}</div>
</div>
</div>
</div>
)}
...
```
{/* ── Profiles ── */}
<Card title="Your Profile" accent={B} right={persons.length<3&&<button className="btn btn-ghost" style={{fontSize:".72rem",padding:".25rem .65rem"}} onClick={addP}>+ Add Person</button>}>
  {persons.map((p,pi)=>(
    ...
    <div className="psec" key={p.id}>
                  ...
                  <div className="sb2 full">
                    ...
                    <div className="g2">
                      <div>
                        ...
                        <div className="hint">≈ {fmt(s.pt-(a.erTax==="pretax"?Math.min(s.pt+s.rt,p.salary/12*(a.erCap/100))*(a.erPct/100):0))}/mo</div>
                      </div>
                      <div>
                        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:".3rem"}}>
                        ...
                        <div className="hint">≈ {fmt(s.rt-(a.erTax==="roth"?Math.min(s.pt+s.rt,p.salary/12*(a.erCap/100))*(a.erPct/100):0))}/mo</div>
                      </div>
                    </div>
                    <div className="hint" style={{marginTop:".4rem",paddingTop:".4rem",borderTop:`1px solid ${BD}`}}>Total employee: <strong>{fmt((a.empPreTaxMode==="pct"?p.salary/12*a.empPreTaxPct/100:a.empPreTaxDollar)+(a.empRothMode==="pct"?p.salary/12*a.empRothPct/100:a.empRothDollar))}/mo</strong></div>
                  </div>
                  <div className="sb2 full">
                    <div className="sb2-hdr">
                    ...
                    <div className="g2">
                      ...
                      <label className="fld">Up to<NI value={a.erCap} min={0} max={20} step={0.5} suffix="% sal" onChange={v=>updA(p.id,a.id,{...a,erCap:v})}/></label>
                    </div>
                    <div className="hint" style={{marginTop:".35rem"}}>≈ <strong>{fmt(a.erTax==="pretax"?s.pt-((a.empPreTaxMode==="pct"?p.salary/12*a.empPreTaxPct/100:a.empPreTaxDollar)):s.rt-((a.empRothMode==="pct"?p.salary/12*a.empRothPct/100:a.empRothDollar)))}/mo</strong> employer match ({a.erTax==="pretax"?"Pre-Tax":"Roth"})</div>
                  </div>
                  <label className="fld">Return<NI value={a.growth} min={0} max={20} step={0.1} suffix="%/yr" onChange={v=>updA(p.id,a.id,{...a,growth:v})}/></label>
                </div>
              ):(
                <div className="acf">
                  {a.type!=="brok"&&(
                  ...
                  <label className="fld full">Return<NI value={a.growth} min={0} max={20} step={0.1} suffix="%/yr" onChange={v=>updA(p.id,a.id,{...a,growth:v})}/></label>
                </div>
              )}
            </div>
          );
      ...
      <div className="add-row">
        ...
        {Object.entries(ATYPES).map(([type,cfg])=>(
          <button key={type} className="btn btn-ghost" style={{fontSize:".7rem",padding:".25rem .6rem"}} onClick={()=>addA(p.id,type)}>+ {cfg.label}</button>
        ))}
      </div>
    </div>
  ))}
...
{/* ── Pension & SS ── */}
<Card title="Pension & Social Security" accent={PS}>
  <div style={{display:"flex",flexDirection:"column",gap:".85rem"}}>
    <div className="eic">
      <div className="eic-hdr"><Toggle on={pension.on} onChange={v=>setPension(s=>({...s,on:v}))}/><span style={{fontWeight:700,fontSize:".88rem",color:N}}>Pension</span>{pension.on&&<span style={{fontSize:".7rem",color:GR,marginLeft:"auto",fontWeight:600}}>≈ {fmt(pensionMo)}/mo at retirement</span>}</div>
      {pension.on&&<div className="eic-f">
        ...
        <label className="fld">COLA<NI value={pension.cola} min={0} max={10} step={0.1} suffix="%/yr" onChange={v=>setPension(s=>({...s,cola:v}))}/></label>
      </div>}
    </div>
    <div className="eic">
      <div className="eic-hdr"><Toggle on={ss.on} onChange={v=>setSS(s=>({...s,on:v}))}/><span style={{fontWeight:700,fontSize:".88rem",color:N}}>Social Security</span>{ss.on&&<span style={{fontSize:".7rem",color:GR,marginLeft:"auto",fontWeight:600}}>≈ {fmt(ssMo)}/mo at retirement</span>}</div>
      {ss.on&&<div className="eic-f">
        ...
        <label className="fld">COLA<NI value={ss.cola} min={0} max={5} step={0.1} suffix="%/yr" onChange={v=>setSS(s=>({...s,cola:v}))}/></label>
      </div>}
    </div>
  </div>
</Card>
...
{/* ── Tax Assumptions ── */}
<Card title="Tax Assumptions" accent={MU}>
  <div style={{display:"flex",flexDirection:"column",gap:".85rem"}}>
    <div>
    ...
    <div>
      ...
      <div className="hint">0% (&lt;$47k income), 15% (most), 20% (high earners). Roth always 0%.</div>
    </div>
    <div>
      <label className="fld" style={{marginBottom:".4rem"}}>Withdrawal Rate</label>
      <SlN value={wRate} min={1} max={10} step={0.1} suffix="%" color={wRate<=4?GR:wRate<=5.5?AM:RD} onChange={setWRate}/>
      <div style={{display:"flex",gap:".4rem",flexWrap:"wrap",marginTop:".5rem"}}>
        {[{v:3,l:"3%"},{v:4,l:"4% Classic"},{v:5,l:"5%"},{v:7,l:"7%"}].map(r=>{
          ...
          return <button key={r.v} onClick={()=>setWRate(r.v)} style={{fontSize:".7rem",padding:".22rem .6rem",borderRadius:5,border:`1.5px solid ${c}55`,background:c+"15",color:c,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",fontWeight:600}}>{r.l}</button>;
        })}
      </div>
    </div>
  </div>
...
```
{/* ── Wealth Map ── */}
<Card title="Wealth Map" sub="Your complete household financial picture at a glance" accent={N}>
  <WealthMap persons={persons} allA={allA} streams={streams}
    baseTotal={baseTotal} basePT={basePT} baseRT={baseRT} baseTX={baseTX}
...
{/* ── Goal Income (backward mode) ── */}
{mode==="backward"&&(
  <Card title="Goal Income" accent={GR} sub="Start here — what income do you want in retirement?">
    ...
    {goalMode==="manual"?(
      <div>
        <label className="fld" style={{marginBottom:".5rem"}}>Target Annual Income in Retirement (today's dollars)
          <div className="hint">We'll inflate this to your retirement date automatically.</div>
        </label>
        ...
        <div style={{display:"flex",gap:".4rem",flexWrap:"wrap",marginTop:".6rem"}}>
          {[{v:totalSal*.6,l:"60% of salary"},{v:totalSal*.8,l:"80% of salary"},{v:totalSal,l:"100% of salary"}].map(r=>(
            <button key={r.v} onClick={()=>setGoalManual(Math.round(r.v))} style={{fontSize:".7rem",padding:".22rem .65rem",borderRadius:5,border:`1.5px solid ${B}44`,background:B+"15",color:B,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",fontWeight:500}}>{r.l}</button>
          ))}
        </div>
      </div>
    ):(
      <div style={{display:"flex",flexDirection:"column",gap:".85rem"}}>
        <div className="info" style={{marginBottom:".25rem"}}>Estimate your monthly expenses in retirement. We'll total them up and inflate to your retirement date.</div>
        {[
          {l:"Housing (rent or mortgage)",v:goalHousing,set:setGoalHousing,hint:"Paid off? Enter HOA/tax/insurance only.",max:10000},
          {l:"Healthcare & Insurance",v:goalHealth,set:setGoalHealth,hint:"Medicare + supplemental avg ~$500–800/mo.",max:5000},
          {l:"Travel & Leisure",v:goalTravel,set:setGoalTravel,hint:"Vacations, hobbies, dining out.",max:5000},
        ...
        <div style={{background:S,border:`1px solid ${BD}`,borderRadius:8,padding:".75rem 1rem"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline"}}>
            <span style={{fontSize:".78rem",color:MU}}>Monthly expenses today</span>
          ...
          <div>
            <label className="fld" style={{marginTop:".65rem",marginBottom:".3rem"}}>Inflation Rate
              <div className="hint">Used to inflate today's expenses to retirement date ({maxRetYrs} yrs away).</div>
            </label>
            <SlN value={goalInflation} min={0} max={6} step={0.1} suffix="%/yr" color={AM} onChange={setGoalInflation}/>
          </div>
...
{/* ── Gap Analysis (backward mode) ── */}
{mode==="backward"&&(
  <Card title="Gap Analysis" accent={onTrack?GR:RD} sub="What you have vs. what you need">
    <div className={`gap-banner ${onTrack?"success":gap>requiredPortfolio*0.5?"danger":"warn"}`} style={{borderRadius:10,padding:"1rem 1.25rem",marginBottom:"1rem",background:onTrack?"#F0FDF4":gap>requiredPortfolio*0.5?"#FEF2F2":"#FFFBEB",border:`1.5px solid ${onTrack?GR+"55":gap>requiredPortfolio*0.5?RD+"55":AM+"55"}`}}>
      <div style={{display:"flex",alignItems:"flex-start",gap:".65rem"}}>
        <span style={{fontSize:"1.5rem",lineHeight:1}}>{onTrack?"✅":"⚠️"}</span>
        <div>
          <div style={{fontWeight:700,fontSize:".95rem",color:onTrack?"#14532D":gap>requiredPortfolio*0.5?"#7F1D1D":"#78350F"}}>
            {onTrack?`You're on track! Projected surplus of ${fmtK(Math.abs(gap))}.`:`Portfolio gap of ${fmtK(Math.abs(gap))} to fund your goal.`}
          </div>
          <div style={{fontSize:".78rem",marginTop:4,color:MU}}>
            {onTrack
              ?`Your projected portfolio of ${fmtK(baseTotal)} exceeds the ${fmtK(requiredPortfolio)} needed to fund ${fmt(goalEstimated/12)}/mo.`
              :`You need ${fmtK(requiredPortfolio)} at retirement to fund ${fmt(goalEstimated/12)}/mo net. You're projected to have ${fmtK(baseTotal)}.`}
          </div>
        </div>
      </div>
    </div>

    <GapMeter have={baseTotal} need={requiredPortfolio} label="Portfolio Funded"/>
    <GapMeter have={totalNetAnnual} need={goalEstimated} label="Income Funded"/>

    <div style={{background:S,border:`1px solid ${BD}`,borderRadius:10,padding:"1rem",marginTop:"1rem"}}>
      <div style={{fontWeight:700,fontSize:".82rem",color:N,marginBottom:".75rem"}}>What You Need</div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:".65rem"}}>
        {[
          {l:"Goal Annual Income",v:fmt(goalEstimated),c:GR},
          {l:"Required Portfolio",v:fmtK(requiredPortfolio),c:N},
          {l:"Projected Portfolio",v:fmtK(baseTotal),c:baseTotal>=requiredPortfolio?GR:RD},
          {l:"Projected Net Income",v:fmt(totalNetAnnual)+"/yr",c:totalNetAnnual>=goalEstimated?GR:AM},
        ].map(x=>(
          <div key={x.l} className="stat" style={{padding:".7rem .85rem"}}>
            <div className="sl">{x.l}</div>
            <div className="sv" style={{color:x.c,fontSize:"1rem"}}>{x.v}</div>
          </div>
        ))}
      </div>
    </div>
    ...
    {!onTrack&&(
      <div style={{marginTop:"1rem"}}>
        ...
        {[
          {label:`Save an extra ${fmt(additionalNeeded)}/mo`,desc:`Bring total monthly savings to ${fmt(currentMonthly+additionalNeeded)}/mo`,icon:"💰",impact:"Closes full portfolio gap"},
          {label:`Retire ${Math.ceil(Math.log(requiredPortfolio/Math.max(baseTotal,1))/Math.log(1.07))} years later`,desc:`Working longer gives investments more time to compound`,icon:"📅",impact:`${Math.ceil(Math.log(requiredPortfolio/Math.max(baseTotal,1))/Math.log(1.07))} additional working years`},
          {label:`Reduce goal to ${fmt((totalNetAnnual)/12)}/mo`,desc:"Adjust lifestyle expectations to match current trajectory",icon:"🎯",impact:"Fully funded with current plan"},
          {label:`Lower withdrawal to ${pct(Math.max(1,(portionFromPortfolio/baseTotal)*100))}`,desc:"Stretch the portfolio with a lower withdrawal rate",icon:"📊",impact:"Reduces income but increases portfolio longevity"},
        ].map((a,i)=>(
          <div key={i} style={{display:"flex",gap:".75rem",alignItems:"flex-start",padding:".75rem",background:i%2===0?S:"#fff",borderRadius:8,marginBottom:".4rem",border:`1px solid ${BD}`}}>
            ...
            <div>
              ...
              <div style={{fontSize:".7rem",color:B,fontWeight:600,marginTop:3}}>→ {a.impact}</div>
            </div>
          </div>
        ))}
      </div>
...
<Card title="Growth Projection"
  ...
  <div style={{fontSize:".76rem",color:MU,marginBottom:".85rem"}}>
    {chartTab==="accum"
      ?<>{CY} → {CY+maxRetYrs} · <strong style={{color:PT}}>Pre-Tax {fmtK(basePT)}</strong> · <strong style={{color:RT}}>Roth {fmtK(baseRT)}</strong> · <strong style={{color:TXC}}>Taxable {fmtK(baseTX)}</strong> · <strong style={{color:N}}>Total {fmtK(baseTotal)}</strong></>
      :<>Accumulation + drawdown at {wRate}% withdrawal · through age {p0.lifeExp}</>}
  </div>
  <ResponsiveContainer width="100%" height={320}>
    <AreaChart data={chartRows} margin={{top:6,right:8,left:8,bottom:0}}>
      <defs>
        {[[PT,"PT"],[RT,"RT"],[TXC,"TX"]].map(([c,k])=>(
          <linearGradient key={k} id={`g${k}`} x1="0" y1="0" x2="0" y2="1">
            <stop offset="5%" stopColor={c} stopOpacity={.2}/><stop offset="95%" stopColor={c} stopOpacity={.02}/>
          </linearGradient>
        ))}
      </defs>
      ...
      <Tooltip content={<Tip isAge={chartTab==="lifetime"}/>}/>
      ...
      {mode==="backward"&&requiredPortfolio>0&&chartTab==="accum"&&(
        <ReferenceLine y={requiredPortfolio} stroke={GR} strokeDasharray="5 3" label={{value:"Goal",position:"insideTopRight",fill:GR,fontSize:11,fontWeight:700}}/>
      )}
      {chartTab==="accum"&&<ReferenceLine x={maxRetYrs} stroke={AM} strokeDasharray="5 3" label={{value:"Retire",position:"insideTopRight",fill:AM,fontSize:11,fontWeight:600}}/>}
      {chartTab==="lifetime"&&<ReferenceLine x={p0.retireAge} stroke={AM} strokeDasharray="5 3" label={{value:"Retire",position:"insideTopRight",fill:AM,fontSize:11,fontWeight:600}}/>}
    </AreaChart>
  </ResponsiveContainer>
  {chartTab==="lifetime"&&lifeRows.length>0&&lifeRows[lifeRows.length-1].TOT===0&&(
    <div className="warn" style={{marginTop:".85rem"}}>⚠️ <span>Portfolio depletes before age {p0.lifeExp} at {wRate}% withdrawal. Consider reducing withdrawal rate or increasing savings.</span></div>
  )}
</Card>
...
<Card title="Retirement Income Breakdown" sub={`At ${wRate}% withdrawal · ${taxRate}% tax on pre-tax · ${ltcgRate}% LTCG on taxable`}>
  <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(170px,1fr))",gap:".75rem",marginBottom:"1rem"}}>
    {[
      {l:"Pre-Tax Accounts",g:portIncome.pt+portIncome.pt*(taxRate/100/(1-taxRate/100)),n:portIncome.pt,sub:`after ${taxRate}% tax`,c:PT,bg:"#EFF6FF"},
      {l:"Roth Accounts",g:portIncome.rt,n:portIncome.rt,sub:"tax-free ✓",c:RT,bg:"#ECFDF5"},
      {l:"Taxable Brokerage",g:portIncome.tx/(1-ltcgRate/100),n:portIncome.tx,sub:`after ${ltcgRate}% LTCG`,c:TXC,bg:"#FFFBEB"},
      ...(pension.on?[{l:"Pension",g:pensionMo*12,n:pensionMo*12,sub:`${pension.cola}% COLA · age ${pension.startAge}`,c:PS,bg:"#F5F3FF"}]:[]),
      ...(ss.on?[{l:"Social Security",g:ssMo*12,n:ssMo*12,sub:`${ss.cola}% COLA · age ${ss.startAge}`,c:SS,bg:"#F0FDFA"}]:[]),
    ].map(b=>(
  ...
  <div className="grand">
    ...
    <div><div className="gl">Income Replacement</div><div className="gv">{totalSal>0?pct(totalNetAnnual/totalSal*100):"—"}</div><div className="gs">target ≥ 80%</div></div>
  </div>
</Card>

{/* ── What-If Explorer ── */}
<Card title="What-If Explorer" sub="Model changes and see the impact on your retirement instantly">
  {/* Tab bar */}
  <div style={{display:"flex",gap:".3rem",flexWrap:"wrap",marginBottom:"1rem",padding:".3rem",background:S,borderRadius:9,border:`1px solid ${BD}`}}>
    {[
      ...
      <button key={t.v} onClick={()=>setWiTab(t.v)}
        style={{flex:"1 1 auto",padding:".38rem .7rem",borderRadius:7,border:"none",
          fontFamily:"'DM Sans',sans-serif",fontWeight:600,fontSize:".74rem",cursor:"pointer",transition:"all .15s",
          background:wiTab===t.v?N:"transparent",color:wiTab===t.v?"#fff":MU}}>
        {t.l}
      </button>
    ))}
  </div>
  ...
  {/* Row header helper */}
  {(() => {
    const baseInc = baseResult.income;
    const basePort = baseResult.portfolio;
    const goal = goalEstimated/12;
    const RowHead = ({cols})=>(
      <div style={{display:"grid",gridTemplateColumns:cols,gap:".4rem",padding:".5rem .8rem",background:N,borderRadius:7,marginBottom:".35rem"}}>
        {["Scenario","Portfolio at Retire","Net Monthly Income","vs. Today","Goal?"].map(h=>(
          <span key={h} style={{fontSize:".65rem",fontWeight:700,color:"rgba(255,255,255,.8)",textTransform:"uppercase",letterSpacing:".04em"}}>{h}</span>
        ))}
      </div>
    );
    const Row = ({label,portfolio,income,isBase,cols})=>{
      const dPort=portfolio-basePort;
      const dInc=income-baseInc;
      const meetsGoal=mode==="backward"&&goal>0&&income/12>=goal;
      return(
        <div style={{display:"grid",gridTemplateColumns:cols,gap:".4rem",padding:".55rem .8rem",
          ...
          <span style={{fontWeight:isBase?700:500,color:isBase?B:TX,fontSize:".78rem"}}>{label}{meetsGoal&&<span style={{marginLeft:5,fontSize:".62rem",color:GR,fontWeight:700}}>✓ GOAL</span>}</span>
          ...
          <span style={{fontSize:".7rem",color:meetsGoal?GR:goal>0?MU:MU}}>
            {mode==="backward"&&goal>0?(meetsGoal?"✓ Met":"−"+fmt(goal-income/12)+"/mo"):"—"}
          </span>
        </div>
      );
    };
    const cols="1.4fr 1.2fr 1.3fr .9fr .75fr";

    if(wiTab==="monthly") return(
      <div>
        <div style={{fontSize:".76rem",color:MU,marginBottom:".75rem",lineHeight:1.5}}>
          What if you increased your monthly contributions by a fixed amount each month, starting today?
          All extra is split 60% pre-tax / 30% Roth / 10% taxable.
        </div>
        <RowHead cols={cols}/>
        {whatIfMonthly.map(r=><Row key={r.extra} label={r.extra===0?"Current Plan":`+${fmt(r.extra)}/mo more`} portfolio={r.portfolio} income={r.income} isBase={r.extra===0} cols={cols}/>)}
      </div>
    );

    if(wiTab==="lumpsum") return(
      <div>
        <div style={{fontSize:".76rem",color:MU,marginBottom:".75rem",lineHeight:1.5}}>
          What if you invested a lump sum of cash today — say, savings sitting in a checking account, a bonus, an inheritance, or proceeds from selling something?
          The lump sum is invested proportionally across your existing accounts.
        </div>
        <div style={{display:"flex",gap:".5rem",alignItems:"center",marginBottom:".85rem",flexWrap:"wrap"}}>
          <span style={{fontSize:".72rem",color:MU,fontWeight:600}}>Or enter a custom amount:</span>
          <NI value={wiLumpSum} min={0} max={2000000} step={1000} prefix="$" onChange={setWiLumpSum} style={{maxWidth:160}}/>
          <span style={{fontSize:".72rem",color:GR,fontWeight:600}}>→ Portfolio: {fmtK(scenarioResult(0,wiLumpSum,0,0).portfolio)} · Income: {fmt(scenarioResult(0,wiLumpSum,0,0).income/12)}/mo</span>
        </div>
        <RowHead cols={cols}/>
        {whatIfLumpSum.map(r=><Row key={r.lump} label={r.lump===0?"Current Plan (no extra cash)":`Invest ${fmtK(r.lump)} today`} portfolio={r.portfolio} income={r.income} isBase={r.lump===0} cols={cols}/>)}
      </div>
    );

    if(wiTab==="retire") return(
      <div>
        <div style={{fontSize:".76rem",color:MU,marginBottom:".75rem",lineHeight:1.5}}>
          What if you retired earlier or later? Working longer gives your investments more time to compound and reduces the drawdown period. Retiring earlier does the opposite — but may be worth it.
        </div>
        <RowHead cols={cols}/>
        {whatIfRetireAge.map(r=><Row key={r.age} label={r.age===p0.retireAge?`Age ${r.age} (Current)`:`Retire at Age ${r.age} (${r.yrs} yrs)`} portfolio={r.portfolio} income={r.income} isBase={r.age===p0.retireAge} cols={cols}/>)}
      </div>
    );

    if(wiTab==="growth") return(
      <div>
        <div style={{fontSize:".76rem",color:MU,marginBottom:".75rem",lineHeight:1.5}}>
          What if your investments grew at a different rate? This models different market environments — from a conservative bond-heavy portfolio to an aggressive equity portfolio.
          Base rate assumed: 7% annually.
        </div>
        <RowHead cols={cols}/>
        {whatIfGrowth.map(r=>(
          <Row key={r.adj} label={r.adj===0?`${r.rate}%/yr (Current)`:r.adj<0?`${r.rate}%/yr (Conservative)`:r.adj<=1?`${r.rate}%/yr (Moderate Growth)`:r.adj<=2?`${r.rate}%/yr (Strong Growth)`:`${r.rate}%/yr (Aggressive)`}
            portfolio={r.portfolio} income={r.income} isBase={r.adj===0} cols={cols}/>
        ))}
      </div>
    );

    if(wiTab==="combined") return(
      <div>
        <div style={{fontSize:".76rem",color:MU,marginBottom:".75rem",lineHeight:1.5}}>
          Combinations of saving more monthly <em>and</em> investing a lump sum. See how these levers compound together over your time horizon.
        </div>
        <RowHead cols={cols}/>
        {whatIfCombined.map((r,i)=><Row key={i} label={r.label} portfolio={r.portfolio} income={r.income} isBase={i===0} cols={cols}/>)}
      </div>
    );
  ...
  {mode==="backward"&&!onTrack&&(
    <div className="success" style={{marginTop:"1rem"}}>
      💡 <span>To hit <strong>{fmt(goalEstimated/12)}/mo</strong> you need <strong>{fmt(additionalNeeded)}/mo</strong> more in contributions ({pct(additionalNeeded/Math.max(totalSal/12,1)*100)} of monthly salary). Use the tabs above to find your path.</span>
    </div>
  )}
</Card>
...
<div className="footer">
  ...
  Pension and Social Security shown in nominal dollars with COLA. This tool does not constitute financial advice. Consult a qualified financial planner.
</div>
```
