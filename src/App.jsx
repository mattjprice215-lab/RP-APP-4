...
mode:“dollar”, contribDollar:300, contribPct:5,
growth:7,
};
}
function mkPerson(n) {
...
const filled   = cl(ratio*100, 0, 100);
...
const pcol     = [N, WMC.income, “#DB2777”];
return (
  ...
  <div style={{ display:"grid", gridTemplateColumns:"1fr auto 1fr", gap:"1.25rem", alignItems:"center", minWidth:480 }}>
    ...
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:8, minWidth:130 }}>
      ...
      <div style={{ display:"flex", gap:16, justifyContent:"center" }}>
        {persons.map((p,i) => <PersonIcon key={p.id} name={p.name} color={pcol[i%3]} size={44} />)}
      </div>
...
// Goal-income state (backward mode)
...
const portionFromPort    = Math.max(0, goalEstimated-pensionSsAnnual);
const blendedTax         = (basePT+baseTX>0)
? ((basePT*taxRate/100 + baseTX*ltcgRate/100)/(basePT+baseTX+Math.max(baseRT,1)))*100
: taxRate;
const grossNeeded        = portionFromPort / (1 - blendedTax/100*0.5);
const requiredPortfolio  = (wRate>0) ? grossNeeded/(wRate/100) : 0;
...
// ── What-If engine ────────────────────────────────────────────────────────
const scenarioResult = useCallback((extraMonthly=0, lumpSum=0, growthAdj=0, retireAgeOverride=null)=>{
...
const baseResult = useMemo(()=>scenarioResult(0,0,0,null),[scenarioResult]);
...
// ── What-If row renderer ──────────────────────────────────────────────────
const WIRow = ({label, portfolio, income, isBase}) => {
const dInc = income - baseResult.income;
const meetsGoal = mode===“backward” && goalEstimated>0 && income/12 >= goalEstimated/12;
return (
<div style={{
...
{label}{meetsGoal&&<span style={{marginLeft:5,fontSize:”.62rem”,color:GR,fontWeight:700}}>✓ GOAL</span>}
</span>
...
{mode===“backward”&&goalEstimated>0 ? (meetsGoal?“✓ Met”:“−”+fmt(goalEstimated/12-income/12)+”/mo”) : “—”}
</span>
</div>
);
};
...
// ── CSS ───────────────────────────────────────────────────────────────────
const css = `@import url('https://fonts.googleapis.com/css2?family=DM+Sans:ital,wght@0,300;0,400;0,500;0,600;0,700;1,400&family=JetBrains+Mono:wght@400;500;600&display=swap'); *,*::before,*::after{box-sizing:border-box;margin:0;padding:0} body{background:#EEF2FA;color:${TX};font-family:'DM Sans',sans-serif;font-size:15px;min-height:100vh} .app{max-width:1380px;margin:0 auto;padding:0 1.5rem 5rem} .topbar{background:${N};position:sticky;top:0;z-index:200;box-shadow:0 2px 16px rgba(0,38,119,.3)} .topbar-inner{max-width:1380px;margin:0 auto;padding:.7rem 1.5rem;display:flex;align-items:center;gap:1rem;justify-content:space-between;flex-wrap:wrap} .logo-box{display:flex;align-items:center;gap:.75rem} .logo-sq{width:34px;height:34px;background:#fff;border-radius:6px;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:14px;color:${N};flex-shrink:0} .logo-text{color:#fff;font-weight:700;font-size:1rem} .logo-sub{color:rgba(255,255,255,.5);font-size:.72rem;margin-top:1px} .mode-toggle{display:flex;background:rgba(255,255,255,.12);border-radius:8px;overflow:hidden;border:1px solid rgba(255,255,255,.2)} .mode-btn{background:none;border:none;color:rgba(255,255,255,.65);padding:.45rem 1.1rem;font-size:.82rem;font-weight:600;cursor:pointer;font-family:'DM Sans',sans-serif;transition:all .15s;white-space:nowrap} .mode-btn.active{background:#fff;color:${N}} .layout{display:grid;grid-template-columns:400px 1fr;gap:1.5rem;margin-top:1.5rem;align-items:start} @media(max-width:1024px){.layout{grid-template-columns:1fr}} .left-col,.right-col{display:flex;flex-direction:column;gap:0} .ni{display:flex;align-items:stretch;background:#fff;border:1.5px solid ${BD};border-radius:7px;overflow:hidden;transition:border-color .15s} .ni:focus-within{border-color:${B}} .ni-a,.ni-b{padding:0 .55rem;background:${S};color:${MU};font-family:'JetBrains Mono',monospace;font-size:.75rem;display:flex;align-items:center;white-space:nowrap;flex-shrink:0} .ni-a{border-right:1.5px solid ${BD}}.ni-b{border-left:1.5px solid ${BD}} .ni input{background:transparent;border:none;outline:none;color:${TX};font-family:'JetBrains Mono',monospace;font-size:.88rem;padding:.42rem .55rem;width:100%;min-width:0;font-weight:500;-moz-appearance:textfield} .ni input::-webkit-outer-spin-button,.ni input::-webkit-inner-spin-button{-webkit-appearance:none} input[type=range]{-webkit-appearance:none;width:100%;height:4px;border-radius:2px;outline:none;cursor:pointer;background:linear-gradient(to right,var(--c,${B}) 0%,var(--c,${B}) var(--p,50%),${BD} var(--p,50%),${BD} 100%)} input[type=range]::-webkit-slider-thumb{-webkit-appearance:none;width:16px;height:16px;border-radius:50%;background:var(--c,${B});border:2.5px solid #fff;box-shadow:0 0 0 2px var(--c,${B})44,0 1px 4px rgba(0,0,0,.1)} input[type=range]::-webkit-slider-thumb:hover{box-shadow:0 0 0 4px var(--c,${B})33} label.fld{display:flex;flex-direction:column;gap:.3rem;font-size:.7rem;color:${MU};font-weight:600;letter-spacing:.03em;text-transform:uppercase} .hint{font-size:.68rem;color:${MU};margin-top:.18rem;text-transform:none;letter-spacing:0} .seg{display:inline-flex;background:${S};border:1.5px solid ${BD};border-radius:7px;overflow:hidden} .sb{background:none;border:none;color:${MU};padding:.28rem .65rem;font-size:.74rem;cursor:pointer;font-family:'DM Sans',sans-serif;font-weight:500;transition:all .15s;white-space:nowrap} .sb.on{background:${N};color:#fff} .sb:not(.on):hover{background:#E8EDF8;color:${TX}} .btn-ghost{background:transparent;border:1.5px solid ${BD};color:${MU};border-radius:7px;padding:.35rem .75rem;font-size:.75rem;font-weight:500;cursor:pointer;font-family:'DM Sans',sans-serif;transition:all .15s} .btn-ghost:hover{border-color:${B};color:${B}} .btn-rm{background:transparent;border:1.5px solid #FECACA;color:${RD};border-radius:6px;padding:.2rem .6rem;font-size:.7rem;cursor:pointer;transition:all .15s;font-family:'DM Sans',sans-serif} .btn-rm:hover{background:#FEF2F2} .ac{background:#fff;border:1.5px solid ${BD};border-top:3px solid var(--c,${B});border-radius:10px;padding:1rem;transition:box-shadow .15s} .ac:hover{box-shadow:0 3px 12px rgba(0,38,119,.08)} .ac-top{display:flex;align-items:center;gap:.45rem;margin-bottom:.8rem;padding-bottom:.7rem;border-bottom:1px solid ${BD}} .ac-name{font-weight:700;font-size:.85rem;color:${N}} .ac-rm{background:none;border:none;color:${MU};cursor:pointer;font-size:.78rem;opacity:.4;transition:opacity .15s;padding:1px 4px;margin-left:auto} .ac-rm:hover{opacity:1;color:${RD}} .acf{display:grid;grid-template-columns:1fr 1fr;gap:.55rem} .full{grid-column:1/-1} .sb2{background:${S};border:1.5px solid ${BD};border-radius:8px;padding:.75rem} .sb2-hdr{display:flex;align-items:center;justify-content:space-between;margin-bottom:.55rem} .stat{background:#fff;border:1.5px solid ${BD};border-radius:9px;padding:.85rem 1rem} .sl{font-size:.67rem;color:${MU};text-transform:uppercase;letter-spacing:.05em;font-weight:600} .sv{font-family:'JetBrains Mono',monospace;font-size:1.15rem;font-weight:700;margin-top:.22rem;color:${TX}} .sbar2{height:3px;background:${BD};border-radius:2px;margin-top:.45rem;overflow:hidden} .sbar2f{height:100%;border-radius:2px;transition:width .4s} .chart-tabs{display:flex;background:${S};border:1.5px solid ${BD};border-radius:7px;overflow:hidden} .chart-tab{background:none;border:none;color:${MU};padding:.35rem .9rem;font-size:.76rem;cursor:pointer;font-family:'DM Sans',sans-serif;font-weight:500;transition:all .15s} .chart-tab.on{background:${N};color:#fff} .info{background:${S};border:1px solid ${BD};border-radius:8px;padding:.7rem 1rem;font-size:.76rem;color:${MU};line-height:1.6} .warn{background:#FEF2F2;border:1.5px solid #FECACA;border-radius:8px;padding:.7rem 1rem;font-size:.76rem;color:#991B1B;display:flex;gap:.45rem;line-height:1.5} .success{background:#F0FDF4;border:1.5px solid #BBF7D0;border-radius:8px;padding:.7rem 1rem;font-size:.76rem;color:#14532D;display:flex;gap:.45rem;line-height:1.5} .grand{background:${N};border-radius:12px;padding:1.25rem 1.5rem;display:grid;grid-template-columns:repeat(3,1fr);gap:1rem} @media(max-width:600px){.grand{grid-template-columns:1fr 1fr}} .gl{font-size:.67rem;color:rgba(255,255,255,.6);text-transform:uppercase;letter-spacing:.06em;font-weight:600} .gv{font-family:'JetBrains Mono',monospace;font-size:1.4rem;font-weight:700;color:#fff;margin-top:.2rem} .gs{font-size:.7rem;color:rgba(255,255,255,.45);margin-top:.15rem} .ib{border-radius:10px;padding:1rem 1.1rem;border:1.5px solid} .ib-l{font-size:.68rem;font-weight:700;text-transform:uppercase;letter-spacing:.05em} .ib-g{font-size:.75rem;color:${MU};margin-top:.25rem} .ib-n{font-family:'JetBrains Mono',monospace;font-size:1.25rem;font-weight:700;margin-top:.1rem} .ib-s{font-size:.67rem;color:${MU};margin-top:.15rem} .eic{background:#fff;border:1.5px solid ${BD};border-radius:10px;padding:1rem} .eic-hdr{display:flex;align-items:center;gap:.7rem;margin-bottom:.75rem} .eic-f{display:grid;grid-template-columns:1fr 1fr 1fr;gap:.55rem} .psec{margin-bottom:1.2rem} .pname{font-weight:700;font-size:.88rem;color:${N};display:flex;align-items:center;gap:.4rem;margin-bottom:.65rem} .pdot{width:8px;height:8px;border-radius:50%} .pc-name{background:transparent;border:none;border-bottom:2px solid ${BD};color:${N};font-family:'DM Sans',sans-serif;font-size:1.1rem;font-weight:700;padding:.2rem 0;outline:none;flex:1;transition:border-color .2s;width:100%} .pc-name:focus{border-color:${B}} .pfields{display:grid;grid-template-columns:1fr 1fr;gap:.65rem;margin-top:.9rem} .add-row{display:flex;gap:.4rem;flex-wrap:wrap;margin-top:.65rem;align-items:center} .add-lbl{font-size:.68rem;color:${MU};text-transform:uppercase;letter-spacing:.04em;font-weight:600} .footer{text-align:center;color:${MU};font-size:.68rem;margin-top:1.5rem;padding:1.25rem;background:#fff;border:1px solid ${BD};border-radius:10px;line-height:1.8} .g2{display:grid;grid-template-columns:1fr 1fr;gap:.75rem} .gAuto{display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:.85rem} @media(max-width:680px){.g2,.gAuto{grid-template-columns:1fr}} .wi-tabs{display:flex;gap:.3rem;flex-wrap:wrap;padding:.3rem;background:${S};border-radius:9px;border:1px solid ${BD};margin-bottom:1rem} .wi-tab{flex:1 1 auto;padding:.38rem .7rem;border-radius:7px;border:none;font-family:'DM Sans',sans-serif;font-weight:600;font-size:.74rem;cursor:pointer;transition:all .15s}`;
...
```
  ...
  <div className="app">
    ...
    <div style={{background:"#fff",border:`1px solid ${BD}`,borderRadius:10,padding:".9rem 1.25rem",marginTop:"1.25rem",marginBottom:"-.25rem",display:"flex",alignItems:"center",gap:"1rem",flexWrap:"wrap"}}>
      <div style={{flex:1}}>
        <div style={{fontWeight:700,fontSize:"1rem",color:N}}>{mode==="forward"?"Forward Planning — What Can I Afford?":"Backward Planning — What Do I Need?"}</div>
        <div style={{fontSize:".78rem",color:MU,marginTop:2}}>
          {mode==="forward"?"Enter everything you have. See exactly what retirement looks like and what changes would mean."
                          :"Start with your income goal. We'll show what it requires, what you have, and what to do differently."}
        </div>
      </div>
    ...
    <div className="left-col">
                    ...
                    {a.type==="401k"?(
                      <div className="acf">
                        ...
                        <div className="sb2 full">
                          <div className="sb2-hdr"><span style={{fontSize:".68rem",fontWeight:700,color:N,textTransform:"uppercase",letterSpacing:".04em"}}>Your Contributions</span></div>
                          <div className="g2">
                            <div>
                              ...
                              <div className="hint">≈ {fmt(a.empPreTaxMode==="pct"?(p.salary/12)*(a.empPreTaxPct/100):a.empPreTaxDollar)}/mo</div>
                            </div>
                            <div>
                              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:".3rem"}}>
                              ...
                              <div className="hint">≈ {fmt(a.empRothMode==="pct"?(p.salary/12)*(a.empRothPct/100):a.empRothDollar)}/mo</div>
                            </div>
                          </div>
                        ...
                        <div className="sb2 full">
                          ...
                          <div className="hint" style={{marginTop:".35rem"}}>≈ <strong>{fmt(s.pt+s.rt-(a.empPreTaxMode==="pct"?(p.salary/12)*(a.empPreTaxPct/100):a.empPreTaxDollar)-(a.empRothMode==="pct"?(p.salary/12)*(a.empRothPct/100):a.empRothDollar))}/mo</strong> employer ({a.erTax==="pretax"?"Pre-Tax":"Roth"})</div>
                        </div>
                        <label className="fld">Return<NI value={a.growth} min={0} max={20} step={.1} suffix="%/yr" onChange={v=>updA(p.id,a.id,{...a,growth:v})}/></label>
                      </div>
                    ):(
                      <div className="acf">
                        {a.type!=="brok"&&(
                        ...
                        <label className="fld full">Return<NI value={a.growth} min={0} max={20} step={.1} suffix="%/yr" onChange={v=>updA(p.id,a.id,{...a,growth:v})}/></label>
                      </div>
                    )}
                  </div>
                );
        ...
        <div style={{display:"flex",flexDirection:"column",gap:".85rem"}}>
          <div className="eic">
            <div className="eic-hdr">
              ...
              {pension.on&&<span style={{fontSize:".7rem",color:GR,marginLeft:"auto",fontWeight:600}}>≈ {fmt(pensionMo)}/mo at retirement</span>}
            </div>
          ...
          <div className="eic">
            <div className="eic-hdr">
              ...
              {ss.on&&<span style={{fontSize:".7rem",color:GR,marginLeft:"auto",fontWeight:600}}>≈ {fmt(ssMo)}/mo at retirement</span>}
            </div>
      ...
      {/* Tax & Withdrawal */}
      <Card title="Tax & Withdrawal Assumptions" accent={MU}>
        <div style={{display:"flex",flexDirection:"column",gap:".85rem"}}>
          <div>
          ...
          <div>
            ...
            <div className="hint">0% (&lt;$47k), 15% (most), 20% (high earners). Roth always 0%.</div>
          </div>
          <div>
    ...
    <div className="right-col">
      ...
      {mode==="backward"&&(
        <Card title="Goal Income" accent={GR} sub="What income do you want in retirement?">
          ...
          {goalMode==="manual"?(
            <div>
              <label className="fld" style={{marginBottom:".5rem"}}>Target Annual Income (today's $)
            ...
            <div style={{display:"flex",flexDirection:"column",gap:".85rem"}}>
              <div className="info">Estimate monthly expenses in retirement. We'll total and inflate to your retirement date.</div>
              {[
                {l:"Housing",               v:goalHousing,  set:setGoalHousing,  hint:"Paid off? Enter HOA/tax/insurance only.", max:10000},
                {l:"Healthcare & Insurance", v:goalHealth,   set:setGoalHealth,   hint:"Medicare + supplemental ~$500–800/mo.",    max:5000},
              ...
              <div style={{background:S,border:`1px solid ${BD}`,borderRadius:8,padding:".75rem 1rem"}}>
                ...
                <label className="fld" style={{marginTop:".65rem",marginBottom:".3rem"}}>Inflation Rate
                  <div className="hint">Inflates today's expenses to retirement date ({maxRetYrs} yrs away).</div>
                </label>
      ...
      {mode==="backward"&&(
        <Card title="Gap Analysis" accent={onTrack?GR:RD} sub="What you have vs. what you need">
          <div style={{borderRadius:10,padding:"1rem 1.25rem",marginBottom:"1rem",
            background:onTrack?"#F0FDF4":gap>requiredPortfolio*.5?"#FEF2F2":"#FFFBEB",
            border:`1.5px solid ${onTrack?GR+"55":gap>requiredPortfolio*.5?RD+"55":AM+"55"}`}}>
            <div style={{display:"flex",alignItems:"flex-start",gap:".65rem"}}>
              <span style={{fontSize:"1.5rem",lineHeight:1}}>{onTrack?"✅":"⚠️"}</span>
              <div>
                <div style={{fontWeight:700,fontSize:".95rem",color:onTrack?"#14532D":gap>requiredPortfolio*.5?"#7F1D1D":"#78350F"}}>
                  {onTrack?`On track! Projected surplus of ${fmtK(Math.abs(gap))}.`:`Portfolio gap of ${fmtK(Math.abs(gap))} to fund your goal.`}
                </div>
                <div style={{fontSize:".78rem",marginTop:4,color:MU}}>
                  {onTrack
                    ?`Your projected ${fmtK(baseTotal)} exceeds the ${fmtK(requiredPortfolio)} needed to fund ${fmt(goalEstimated/12)}/mo.`
                    :`You need ${fmtK(requiredPortfolio)} at retirement. You're projected to have ${fmtK(baseTotal)}.`}
                </div>
              </div>
            </div>
          </div>
          ...
          {!onTrack&&(
            <div style={{marginTop:"1rem"}}>
              ...
              {[
                {icon:"💰",label:`Save an extra ${fmt(additionalNeeded)}/mo`,  desc:`Bring total monthly savings to ${fmt(currentMonthly+additionalNeeded)}/mo`},
                {icon:"📅",label:`Retire a few years later`,                   desc:`More compounding time. Use the What-If Explorer to find the right age.`},
                {icon:"🎯",label:`Reduce goal to ${fmt(totalNetAnnual/12)}/mo`, desc:`Match current trajectory — adjust lifestyle expectations`},
                {icon:"📊",label:`Lower withdrawal rate`,                       desc:`Reduces income but extends portfolio longevity significantly`},
              ].map((a,i)=>(
      ...
      <Card title="Growth Projection"
        ...
        {chartTab==="lifetime"&&lifeRows.length>0&&lifeRows[lifeRows.length-1].TOT===0&&(
          <div className="warn" style={{marginTop:".85rem"}}>⚠️ <span>Portfolio depletes before age {p0.lifeExp} at {wRate}% withdrawal. Consider reducing the rate or increasing savings.</span></div>
        )}
      </Card>
      ...
      <Card title="Retirement Income Breakdown" sub={`At ${wRate}% withdrawal · ${taxRate}% tax on pre-tax · ${ltcgRate}% LTCG on taxable`}>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(170px,1fr))",gap:".75rem",marginBottom:"1rem"}}>
          {[
            {l:"Pre-Tax Accounts",  g:portIncome.pt*(1+taxRate/100/(1-taxRate/100+.001)), n:portIncome.pt, sub:`after ${taxRate}% tax`,     c:PT,  bg:"#EFF6FF"},
      ...
      <Card title="What-If Explorer" sub="Model changes and see the retirement impact instantly">
        ...
        {wiTab==="monthly"&&(
          <div>
            ...
            <WIHead/>
            {whatIfMonthly.map(r=><WIRow key={r.extra} label={r.extra===0?"Current Plan":`+${fmt(r.extra)}/mo more`} portfolio={r.portfolio} income={r.income} isBase={r.extra===0}/>)}
          </div>
        )}
        ...
        {wiTab==="lumpsum"&&(
          <div>
            ...
            <div style={{display:"flex",gap:".5rem",alignItems:"center",marginBottom:".85rem",flexWrap:"wrap"}}>
              ...
              <span style={{fontSize:".72rem",color:GR,fontWeight:600}}>→ {fmtK(scenarioResult(0,wiLumpSum,0,null).portfolio)} portfolio · {fmt(scenarioResult(0,wiLumpSum,0,null).income/12)}/mo</span>
            </div>
            <WIHead/>
            {whatIfLumpSum.map(r=><WIRow key={r.lump} label={r.lump===0?"No extra cash":`Invest ${fmtK(r.lump)} today`} portfolio={r.portfolio} income={r.income} isBase={r.lump===0}/>)}
          </div>
        )}

        {wiTab==="retire"&&(
          <div>
            <div className="info" style={{marginBottom:".75rem"}}>Working longer gives investments more time to compound and reduces the drawdown period. Retiring earlier does the opposite — but may be worth it to you.</div>
            <WIHead/>
            {whatIfRetireAge.map(r=><WIRow key={r.age} label={r.age===p0.retireAge?`Age ${r.age} (Current)`:`Retire at Age ${r.age}`} portfolio={r.portfolio} income={r.income} isBase={r.age===p0.retireAge}/>)}
          </div>
        )}

        {wiTab==="growth"&&(
          <div>
            <div className="info" style={{marginBottom:".75rem"}}>Different return rates model different portfolio strategies. Base: 7%/yr. Conservative (bonds-heavy) ≈ 5%. Aggressive (equity-heavy) ≈ 9–10%.</div>
            <WIHead/>
            {whatIfGrowth.map(r=>(
              <WIRow key={r.adj} label={`${r.rate}%/yr${r.adj===0?" (Current)":r.adj<0?" (Conservative)":r.adj<=1?" (Moderate)":r.adj<=2?" (Strong)":"(Aggressive)"}`}
        ...
        {wiTab==="combined"&&(
          <div>
            <div className="info" style={{marginBottom:".75rem"}}>Combinations of extra monthly savings and a one-time lump sum investment — showing how these levers compound together.</div>
            <WIHead/>
            {whatIfCombined.map((r,i)=><WIRow key={i} label={r.label} portfolio={r.portfolio} income={r.income} isBase={i===0}/>)}
          </div>
        )}

        {mode==="backward"&&!onTrack&&(
          <div className="success" style={{marginTop:"1rem"}}>
            💡 <span>To hit <strong>{fmt(goalEstimated/12)}/mo</strong> you need approximately <strong>{fmt(additionalNeeded)}/mo</strong> more ({pct(additionalNeeded/Math.max(totalSal/12,1)*100)} of monthly salary). Use the tabs above to find your path.</span>
          </div>
        )}
      </Card>
